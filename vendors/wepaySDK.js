 ...
 * @param {object} options is an object but only takes custom_style, show_labels, show_error_messages, show_error_messages_when_unfocused, and show_placeholders properties
...
WePay.createCreditCardIframe = WePay.createCreditCardIframe || function (iframe_container_id, options) {
    ...
    if (typeof options !== "object") {
        ...
        for (var property in options){
            if (options.hasOwnProperty(property)) {
                WePay._internal.allowOptions(property, ['custom_style', 'show_labels', 'show_placeholders', 'show_error_messages', 'show_error_messages_when_unfocused']);
            }
        }
    }
    ...
    // Why do we calculate the default credit card iframe height base on the input elements?             
    // When a partner uses the height property in custom_style base object, each of the input element will apply the px                
    // The entire content of cc iframe will change base on that when the page load. Wepay.js listens to the                
    // resize postMessage event from cc iframe to update the final entire iframe content height. However, it's not good enough.
    // User will see a transition from default height to final entire iframe content height. So this try/catch block is to pre-calculate                
    // the entire cc iframe content height. It provides a better user experience.
            ...
            "details": [
                {
                    ...
                    "message": "This account is restricted by the financial institution selected. " +
                        "User Account can’t be supported by Plaid."
                }
            ]
        };
    ...
    this.iframe = function () {
        ...
        var _receiveMessageHandler = function (event) {
            ...
            if (event_data.event_id === "wepay-token" && event_data.ref_id === props.ref_id && event_data.data != null) {
                ...
                // send acknowledgment message back to the iframe so iframe knows that tokenization.js recieve the wepay-size event
