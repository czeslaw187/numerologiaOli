export class ServicesUs {
    ...
    servicesHandler() {
        $('#buyOptions button').on('click', (e)=> {
            $('#payment').remove()
            let cena = ''
            let tytul = $('#buyOptions').find(e.target).parent().parent().find('h4').html()
            cena = $('#buyOptions').find(e.target).parent().find('span').html()
            console.log(cena)
            this.servicesBck()
            $('#mainContent').append(`<div id="payment" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="my-modal-title" aria-hidden="true">
            ...
            paypal.Buttons({
                ...
                createOrder: function(data, actions) {
                  return actions.order.create({
                    purchase_units: [{
                      amount: {
                        value: cena // Can reference variables or functions. Example: `value: document.getElementById('...').value`
                      }
                    }]
                  });
                },
    ...
    displaySectionOpinions() {
        ...
        return `<div class="card mt-5 mt-md-0 w-100 mx-auto noBar h-auto">
                    <div class="card-header"><h3>Zostaw Opinię</h3></div>
                    <span id="opinionErr"></span>                    
                    <form class="form-group contactForm mx-3">
                        <label for="opinionName">Imię/Nick</label>
                        <input type="text" name="opinionName" id="opinionName" class="form-control" required />
                        <label for="yourOpinion">Twoja Opinia</label>
                        <textarea name="yourOpinion" id="yourOpinion" class="form-control" columns="10" rows="4" required></textarea>
                        <button role="button" class="btn btn-secondary mt-2 pull-right" id="submitOpinion">Wyślij</button>
                    </form>
                </div>
                <div class="card mt-5 w-100 mx-auto noBar">
                    <div class="card-body">
